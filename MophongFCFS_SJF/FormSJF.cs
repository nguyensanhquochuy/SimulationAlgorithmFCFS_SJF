...
namespace MophongFCFS_SJF
{
    public partial class FormSJF : Form
    {
        ...
        public void DieuPhoiSJF_NonPreemptive()  // Điều phối tiến trình theo theo SJF NonPreemptive
        {
            int[] f = new int[n]; // danh dau kiém tra q trinh da h6n thanh hay chua
            int st = 0, tot = 0;
            _sumWaitTime = 0; _sumTurnaroundTime = 0;
            ...
            {
                f[i] = 0;
                _dsTT[i].FinishTime = 0;
                _dsTT[i].TurnaroundTime = 0;
                _dsTT[i].WaitTime = 0;
            }
            while (true)
            {
                int c = n, min = 999;
                if (tot == n) // nếu tot = tổng số tiến trình => vòng lăp kết thúc
                {
                    break;
                }
                for (int i = 0; i < n; i++)
                {
                    /* *nếu thời gian xuất hiện của tiến trinh thứ i <= thời điểm hệ thống và được đánh
                    dấu f = 0 và thời gian thực hiện bt<min, *Tiến trình đó sẽ dừng thưc hiện đầu tiên */
                    if ((_dsTT[i].ArrivalTime <= st) && (f[i] == 0) && (_dsTT[i].BurstTime < min))
                    {
                        min = _dsTT[i].BurstTime;
                        c = i;
                    }
                }
                /* Nếu c==n nghĩa là không có tiến trình nào thỏa điều kiện trên nên tăng thời gian hệ thống */
                ...
                {
                    _dsTT[c].FinishTime = st + _dsTT[c].BurstTime;
                    st += _dsTT[c].BurstTime;
                    _dsTT[c].TurnaroundTime = _dsTT[c].FinishTime - _dsTT[c].ArrivalTime;
                    _dsTT[c].WaitTime = _dsTT[c].TurnaroundTime - _dsTT[c].BurstTime;
                    f[c] = 1;
                    tot++;
                }
            }
        ...
        public void SapXepTT_BurstTime()     // Phương thức sắp xếp tăng dần các tiến trình theo thời gian CPU Burst 
        {
            for (int i = 0; i < n - 1; i++)
            {
                for (int j = i + 1; j < n; j++)
                {
                    if (_dsTT[i].BurstTime > _dsTT[j].BurstTime)
                    {
                        TienTrinh temp = new TienTrinh();
                        temp = _dsTT[i];
                        _dsTT[i] = _dsTT[j];
        ...
        public void SapXepTT_FinishTime()   // Phương thức sắp xếp tăng dần các tiến trình theo thời gian hoàn thành 
        {
            for (int i = 0; i < n - 1; i++)
            {
                for (int j = i + 1; j < n; j++)
                {
                    if (_dsTT[i].FinishTime > _dsTT[j].FinishTime)
                    {
                        TienTrinh temp = new TienTrinh();
                        temp = _dsTT[i];
                        _dsTT[i] = _dsTT[j];
